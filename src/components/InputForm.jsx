...
export default function InputForm({ departments, setDepartments, labs, setLabs, subjects, setSubjects, faculties, setFaculties, classStructure, setClassStructure, subjectConstraints, setSubjectConstraints }) {
    ...
    const [specificLabs, setSpecificLabs] = useState([]);
    const [practicalDurationSlots, setPracticalDurationSlots] = useState('2');
    const [activeConstraintTab, setActiveConstraintTab] = useState('joint');
    ...
    const handleLabSelection = (labShortForm) => {
        setSpecificLabs(prev => prev.includes(labShortForm) ? prev.filter(shortForm => shortForm !== labShortForm) : [...prev, labShortForm]);
    };
    ...
    const handleAddLab = () => { if (!labName || !labShortForm) return; setLabs(prevLabs => [...prevLabs, { id: Date.now(), name: labName, shortForm: labShortForm.toUpperCase() }]); setLabName(''); setLabShortForm(''); };
    const handleAddFaculty = () => { if (!facultyName) return; setFaculties(prevFaculties => [...prevFaculties, { id: Date.now(), department: currentDept, title: facultyTitle, name: facultyName, subjectsByYear: facultySubjects }]); setFacultyName(''); setFacultyTitle('Prof.'); setFacultySubjects({}); setFacultyYears({ 2: false, 3: false, 4: false }); };
    ...
    const handleAddConstraint = () => { if (!constraintYear || !constraintSubject) return alert("Please select a year and subject."); let newConstraint; if (activeConstraintTab === 'joint') { newConstraint = { id: Date.now(), type: 'joint', department: currentDept, year: constraintYear, subjectShortForm: constraintSubject, sessionType: constraintSessionType, assignedFaculty: constraintFaculty.map(id => faculties.find(f => f.id === id)?.name || ''), notes: constraintNotes }; } else { if (!slotConstraintDay || !slotConstraintTime) return alert("Please select a day and a specific time slot."); const [start, end] = slotConstraintTime.split('-'); newConstraint = { id: Date.now(), type: 'slot', department: currentDept, year: constraintYear, subjectShortForm: constraintSubject, sessionType: constraintSessionType, day: slotConstraintDay, startTime: start.trim(), endTime: end.trim() }; } setSubjectConstraints(prev => [...prev, newConstraint]); setConstraintSubject(''); setConstraintFaculty([]); setConstraintNotes(''); setSlotConstraintTime(''); };
    
    const isFormDisabled = !currentDept;
    const selectedDeptForSlots = departments.find(d => d.name === currentDept);

    const practicalSlotOptions = useMemo(() => {
        if (activeConstraintTab !== 'slot' || constraintSessionType !== 'practical' || !selectedDeptForSlots || !constraintSubject) return [];
        const subject = subjects.find(s => s.shortForm === constraintSubject && s.year === constraintYear);
        if (!subject?.practicalDetails?.durationSlots) return [];
        const duration = parseInt(subject.practicalDetails.durationSlots, 10);
        if (duration < 1) return [];

        const baseSlots = selectedDeptForSlots.timeSlots['Monday'].filter(s => s.type === 'lecture');
        const combinedSlots = [];
        for (let i = 0; i <= baseSlots.length - duration; i++) {
            let isConsecutive = true;
            for (let j = 0; j < duration - 1; j++) { if (baseSlots[i + j].end !== baseSlots[i + j + 1].start) { isConsecutive = false; break; } }
            if (isConsecutive) combinedSlots.push({ start: baseSlots[i].start, end: baseSlots[i + duration - 1].end });
        }
        return combinedSlots;
    }, [activeConstraintTab, constraintSessionType, constraintSubject, constraintYear, subjects, selectedDeptForSlots]);
        ...
        <main className="container mx-auto px-6 py-8">
            ...
            <div className={`space-y-8 ${isFormDisabled ? 'opacity-50 cursor-not-allowed' : ''}`}>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div className="space-y-8">
                        <section className="bg-white p-6 rounded-lg shadow"><h2 className="text-xl font-semibold mb-4 border-b pb-2">4. Subjects for <span className="text-indigo-600 font-bold">{currentDept}</span></h2>
                            ...
                            {classType !== 'theory' && <div className="p-3 bg-gray-50 rounded-md border space-y-3 mt-4">
                                <SelectField label="Practical Duration (in slots)" value={practicalDurationSlots} onChange={e => setPracticalDurationSlots(e.target.value)} disabled={isFormDisabled}><option value="0">0</option><option value="1">1</option><option value="2">2</option></SelectField>
                                <label className="block text-sm font-medium pt-2 border-t">Practical Type:</label>
                                <div className="space-y-2">
                                    <div className="flex items-center"><input type="radio" id="prac_lab" name="pracType" value="specific_lab" checked={practicalType === 'specific_lab'} onChange={e => setPracticalType(e.target.value)} disabled={isFormDisabled} /><label htmlFor="prac_lab" className="ml-2 text-sm">Requires specific lab(s)</label></div>
                                    {practicalType === 'specific_lab' && <div className="pl-5 border-l-2 ml-2"><label className="block text-sm font-medium my-2">Select Labs:</label><div className="grid grid-cols-2 gap-2">{labs.map(lab => (<div key={lab.id} className="flex items-center"><input type="checkbox" id={`lab-select-${lab.id}`} checked={specificLabs.includes(lab.shortForm)} onChange={() => handleLabSelection(lab.shortForm)} disabled={isFormDisabled} /><label htmlFor={`lab-select-${lab.id}`} className="ml-2 text-sm">{lab.shortForm}</label></div>))}</div></div>}
                ...
                <section className="bg-white p-6 rounded-lg shadow"><h2 className="text-xl font-semibold mb-4 border-b pb-2">6. Subject Constraints</h2>
                    ...
                    {activeConstraintTab === 'slot' && <div>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4"><SelectField label="Year" value={constraintYear} onChange={e => setConstraintYear(e.target.value)} disabled={isFormDisabled}><option value="2">SY</option><option value="3">TY</option><option value="4">Final</option></SelectField><SelectField label="Subject" value={constraintSubject} onChange={e => setConstraintSubject(e.target.value)} disabled={isFormDisabled}><option value="">-- Select --</option>{subjects.filter(s => s.department === currentDept && s.year === constraintYear).map(s => <option key={s.shortForm} value={s.shortForm}>{s.shortForm}</option>)}</SelectField><SelectField label="Session Type" value={constraintSessionType} onChange={e => setConstraintSessionType(e.target.value)} disabled={isFormDisabled}><option value="lecture">Theory</option><option value="practical">Practical</option></SelectField></div>
                        <div className="grid grid-cols-2 gap-4 mt-4">
                            <SelectField label="Day" value={slotConstraintDay} onChange={e => setSlotConstraintDay(e.target.value)} disabled={isFormDisabled}>{['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday'].map(day => <option key={day} value={day}>{day}</option>)}</SelectField>
                            {constraintSessionType === 'lecture' && <SelectField label="Time Slot" value={slotConstraintTime} onChange={e => setSlotConstraintTime(e.target.value)} disabled={isFormDisabled || !selectedDeptForSlots}><option value="">-- Select Slot --</option>{selectedDeptForSlots?.timeSlots['Monday'].filter(s => s.type === 'lecture').map(slot => <option key={slot.start} value={`${slot.start} - ${slot.end}`}>{`${slot.start} - ${slot.end}`}</option>)}</SelectField>}
                            {constraintSessionType === 'practical' && <SelectField label="Time Slot" value={slotConstraintTime} onChange={e => setSlotConstraintTime(e.target.value)} disabled={isFormDisabled || !practicalSlotOptions.length}><option value="">-- Select Slot --</option>{practicalSlotOptions.map(slot => <option key={slot.start} value={`${slot.start} - ${slot.end}`}>{`${slot.start} - ${slot.end}`}</option>)}</SelectField>}
                        </div>
                    </div>}
