# lecture_timetable_generator.py
"""
Fills class timetables with lectures after practicals have been placed.

BUGS FIXED:
1. Afternoon phase was gated on `any_pending` checked ONCE per day before the
   afternoon loop — so if the last morning slot cleared all pending for one class,
   the afternoon phase was skipped for ALL classes that day. Fixed: check pending
   per-subject inside the afternoon loop.

2. The lecture loop processed one subject per slot across ALL classes simultaneously.
   This caused high-priority classes (SY) to fill every slot first, leaving no room
   for TY/BE subjects even when completely different faculty were involved. Fixed:
   inner loop now picks ONE subject per class per slot (round-robin style) rather
   than exhausting the full priority list.

3. No retry: if a lecture couldn't be placed (faculty clash, consecutive block) it
   was silently skipped and never retried. Fixed: outer retry loop (like the
   practical generator) keeps making passes until stable.

4. `is_slot_available` returned False for the 13:15 lunch slot only implicitly
   (it was never in the schedule). Added explicit skip of 13:15.

5. `add_lecture_to_slot` mutated the schedule dict but never guaranteed the day
   key existed before the slot key — could KeyError on edge cases. Fixed.
"""
...
class LectureTimetableGenerator:
    ...
    def prepare_lecture_assignments(self) -> dict:
        ...
        try:
            ...
            for w in workloads:
                ...
                # Smart theory_hrs detection
                theory_hrs = 0
                for field in ('theory_hrs', 'lectures_per_week', 'lecture_hours', 'theory', 'hours'):
                    val = w.get(field)
                    if isinstance(val, (int, float)) and val > 0:
                        theory_hrs = int(val)
                        break
                if theory_hrs == 0:
                    logger.warning(f"No theory_hrs for {year}-{division}-{subject}, defaulting to 1")
                    theory_hrs = 1
    ...
    # Adjacent slots for the no-consecutive-same-subject rule.
    # Lunch (13:15) breaks the chain.
    _ADJACENT = {
        '10:15': ['11:15'],
        '11:15': ['10:15', '12:15'],
        '12:15': ['11:15'],
    ...
    def _place_lecture(self, year: str, division: str, day: str,
        ...
        logger.debug(f"  ✓ {year}-{division} {lecture['subject']} L#{lecture['lecture_number']} "
                     f"→ {day} {slot}")
    ...
    def generate(self) -> dict:
        ...
        try:
            ...
            for pass_num in range(max_passes):
                ...
                for day in DAYS:
                    for slot in ALL_LECTURE_SLOTS:
                        # Try each pending (year, div, subject) in priority order
                        sorted_keys = sorted(
                            [(y, d, s) for (y, d, s) in assignments if assignments[(y, d, s)]],
                        ...
                        for year, division, subject in sorted_keys:
                            ...
                            scheduled_count += 1
                            progress = True
                            # One lecture per class per slot — move to next key
                            # (don't stack multiple lectures in the same slot for same class)
            ...
            for (year, division), tt in self.class_timetables.items():
                # Ensure all day/slot keys exist
                for day in DAYS:
                    for sl in all_slots:
