...
export default function Sidebar({ onTabChange }) {
  ...
  const menuItems = [
    ...
    (item) =>
      !item.role || user?.role?.toLowerCase() === item.role.toLowerCase(),
  );
  ...
  return (
    ...
    >
      <div
        className={`p-6 flex items-center border-b border-slate-700 ${isCollapsed ? "justify-center" : "justify-between"}`}
      >
        <div
          className={`overflow-hidden whitespace-nowrap transition-all duration-300 ease-in-out ${isCollapsed ? "max-w-0 opacity-0 hidden" : "max-w-[200px] opacity-100 flex-1"}`}
        >
            ...
            return (
              ...
              >
                ...
                >
                  <Icon
                    size={24}
                    className={`shrink-0 ${activeTab === item.id ? "text-blue-400" : "text-slate-500"}`}
                  />
                  <span
      ...
      <div className="p-4 border-t border-slate-800 bg-slate-900/50 backdrop-blur-sm">
        <div
          className={`flex items-center ${isCollapsed ? "justify-center" : "gap-3"} p-2 rounded-xl bg-slate-800/30`}
        >
