...
import { getLabSessions, saveLabSessions } from '../services/labSettingsService';
...
export default function LabsData() {
  ...
  // Session CRUD States
  const [labSessions, setLabSessions] = useState(getLabSessions());
  const [isSessionModalOpen, setIsSessionModalOpen] = useState(false);
  const [editingSessionId, setEditingSessionId] = useState(null);
  const [sessionFormData, setSessionFormData] = useState({ startTime: '', endTime: '' });
  ...
  const loadLabs = async () => {
    try {
      ...
      const sorted = labsArray.sort((a, b) => 
  ...
  const handleOpenSessionModal = (session = null) => {
    if (session) {
      setSessionFormData({ startTime: session.startTime, endTime: session.endTime });
  ...
  const handleSaveSession = () => {
    ...
    let updated;
    if (editingSessionId) {
      updated = labSessions.map(s => s.id === editingSessionId ? { ...s, ...sessionFormData } : s);
    } else {
  ...
  return (
    <div className="p-8 max-w-[1600px] mx-auto">
      ...
      <div className="flex items-center justify-between mb-10">
        <div>
          <h2 className="text-4xl font-black text-slate-800 tracking-tight">Labs Management</h2>
          <p className="text-slate-500 mt-2 font-medium">Manage academic laboratories and session timings</p>
        </div>
        <button
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
            ...
            <div className="p-6 space-y-3">
              {labSessions.length === 0 ? (
                <div className="text-center py-6 text-slate-400 text-sm italic">No slots defined</div>
              ) : (
                labSessions.map((session, index) => (
                  <div key={session.id} className="group relative flex items-center justify-between p-4 rounded-2xl bg-slate-50 border border-slate-100 hover:border-blue-200 hover:bg-white transition-all hover:shadow-md">
                    <div className="flex flex-col">
                      <span className="text-[10px] uppercase font-bold text-slate-400 tracking-widest leading-none mb-1">Slot {index + 1}</span>
                    ...
                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => handleOpenSessionModal(session)} className="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg"><Edit2 size={14} /></button>
                      <button onClick={() => handleDeleteSession(session.id)} className="p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg"><Trash2 size={14} /></button>
                    </div>
                  </div>
                ))
              )}
          ...
          <div className="bg-white rounded-3xl shadow-md border border-slate-200 overflow-hidden">
            <table className="w-full">
              <thead className="bg-slate-50 border-b border-slate-200/60">
                <tr>
                  <th className="px-8 py-5 text-left text-sm font-bold text-slate-700 w-[45%]">Lab Name</th>
                  <th className="px-8 py-5 text-left text-sm font-bold text-slate-700 w-[35%]">Short Form</th>
                  <th className="px-8 py-5 text-right text-sm font-bold text-slate-700 w-[20%]">Actions</th>
                </tr>
              </thead>
                ...
                {loading ? (
                  [...Array(4)].map((_, idx) => (
                    <tr key={idx} className="animate-pulse">
                      <td className="px-8 py-6"><div className="h-4 bg-slate-200 rounded w-48"></div></td>
                      <td className="px-8 py-6"><div className="h-4 bg-slate-200 rounded w-24"></div></td>
                      <td className="px-8 py-6 text-right"><div className="w-16 h-8 bg-slate-200 rounded-lg ml-auto"></div></td>
                    </tr>
                  ))
                ) : (
                  labs.map((lab) => (
                    <tr key={lab._id || lab.id} className="hover:bg-slate-50/80 transition-colors">
                      <td className="px-8 py-6 font-semibold text-slate-800 tracking-tight">{lab.name}</td>
                      ...
                      <td className="px-8 py-6 text-right">
                        <div className="flex items-center justify-end gap-2">
                          <button onClick={() => handleOpenLabModal(lab)} className="p-2.5 text-blue-600 hover:bg-blue-50 rounded-xl transition-colors"><Edit2 size={18} /></button>
                          <button onClick={() => handleDeleteLab(lab._id || lab.id)} className="p-2.5 text-red-500 hover:bg-red-50 rounded-xl transition-colors"><Trash2 size={18} /></button>
                        </div>
                      </td>
                    </tr>
                  ))
      ...
      >
        <form onSubmit={handleLabSubmit} className="space-y-6">
          <div className="space-y-2">
            <label className="text-sm font-bold text-slate-700 ml-1">Lab Name</label>
            <input
              type="text"
              value={formData.name}
          ...
          <div className="space-y-2">
            <label className="text-sm font-bold text-slate-700 ml-1">Short Code</label>
            <input
              type="text"
          ...
          <button type="submit" className="w-full py-4 bg-slate-900 text-white rounded-2xl font-bold shadow-lg hover:bg-slate-800 transition-all flex items-center justify-center gap-2">
            <Save size={20} />
      ...
      >
        <div className="space-y-6">
          ...
          <button onClick={handleSaveSession} className="w-full py-4 bg-blue-600 text-white rounded-2xl font-bold shadow-lg hover:bg-blue-700 transition-all flex items-center justify-center gap-2">
            <Save size={20} />
            Save Session
          </button>
        </div>
