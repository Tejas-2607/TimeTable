...
export default function Login() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (status === 404) {
                setIsRegister(true);
                setError('Faculty record not found. Please register by filling in your full name and short name.');
            } else if (status === 401) {
                setError(serverError || 'Invalid password. Please check your credentials.');
            } else if (status >= 500) {
                setError('Server error. Please wait a moment or contact support.');
            } else {
                setError(serverError || 'Authentication failed. Please check your connection or credentials.');
            }
        } finally {
            setLoading(false);
        }
                ...
                <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-3xl shadow-2xl overflow-hidden">
                    <div className="p-8">
                        <div className="text-center mb-10">
                            ...
                            <h1 className="text-3xl font-bold text-white mb-2 tracking-tight">Schedulo</h1>
                            ...
                            >
                                {loading ? (
                                    ...
                                    <>
                                        ...
                                        {!isRegister ? <LogIn size={18} className="group-hover:translate-x-1 transition-transform" /> : <UserPlus size={18} />}
                                    </>
                                )}
                            </button>
                        </form>
