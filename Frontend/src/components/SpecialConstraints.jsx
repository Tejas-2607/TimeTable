...
import { getConstraints, addConstraint, deleteConstraint } from '../services/constraintsService';
...
import {
    Plus, Trash2, ShieldAlert, Clock, Calendar,
    User, Info, Loader2, CheckCircle2, AlertCircle,
    BookOpen, Layers
} from 'lucide-react';
...
export default function SpecialConstraints() {
    ...
    const [formData, setFormData] = useState({
        type: 'preferred_off',
        faculty_name: user?.role === 'faculty' ? user.name : '',
        day: 'Monday',
        time_slot: '',
        description: '',
        year: '',
    ...
    const loadData = async () => {
        setIsLoading(true);
        try {
            const [constraintsData, facultiesData, timingsData, subjectsData, structureData] = await Promise.all([
                getConstraints(),
                getFaculties(),
            ...
            if (timingsData && timingsData.slots) {
                setTimings(timingsData.slots.map(s => s.startTime));
                if (timingsData.slots.length > 0) {
                    setFormData(prev => ({ ...prev, time_slot: timingsData.slots[0].startTime }));
                }
            }

            // Initialize year/div if possible
            const yearsSet = Object.keys(structureData || {}).filter(k => k !== '_id');
            if (yearsSet.length > 0) {
                const firstYear = yearsSet[0].toUpperCase();
                setFormData(prev => ({ ...prev, year: firstYear }));

                const yearData = structureData[yearsSet[0]] || structureData[firstYear.toLowerCase()];
                const numDivs = yearData?.num_divisions || 0;
                const divs = Array.from({ length: numDivs }, (_, i) => String.fromCharCode(65 + i));
    ...
    const activeYears = Object.keys(classStructure).filter(k => k !== '_id').map(y => y.toUpperCase()).sort();

    const getYearData = (year) => {
        if (!year) return null;
        return classStructure[year.toLowerCase()] || classStructure[year.toUpperCase()] || null;
    };

    const currentYearData = getYearData(formData.year);
    const numDivs = currentYearData?.num_divisions || 0;
    const currentDivisions = Array.from({ length: numDivs }, (_, i) => String.fromCharCode(65 + i));
    const currentSubjects = subjectsByYear[formData.year?.toLowerCase()] || subjectsByYear[formData.year?.toUpperCase()] || [];
    ...
    if (isLoading) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[400px]">
                ...
                <p className="text-slate-600 font-medium">Loading constraints data...</p>
            </div>
        );
    }
        ...
        <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
            <div className="mb-8">
                ...
                <p className="text-slate-500 mt-2">
                    Define preferred off-times or fixed lecture timings. The generator will prioritize these settings.
                </p>
            </div>
                ...
                <div className="xl:col-span-1">
                    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 sticky top-8">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div>
                                <label className="block text-sm font-semibold text-slate-700 mb-1">Constraint Type</label>
                                <select
                                    value={formData.type}
                                ...
                                >
                                    ...
                                    <option value="fixed_time">Fixed Lecture/Practical Time</option>
                                </select>
                            </div>

                            {user?.role === 'admin' ? (
                                <div>
                                    <label className="block text-sm font-semibold text-slate-700 mb-1">Faculty Name</label>
                                    <select
                                ...
                                <div className="space-y-4 pt-2 border-t border-slate-100">
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <select
                                                value={formData.year}
                                                onChange={(e) => setFormData({ ...formData, year: e.target.value, division: '', subject: '' })}
                                                className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500 transition-all outline-none"
                                                required
                                            >
                                                {activeYears.map(y => (
                                    ...
                                    <div>
                                        ...
                                        >
                                            ...
                                            {currentSubjects.map(s => (
                                                <option key={s.short_name} value={s.short_name}>{s.name} ({s.short_name})</option>
                                            ))}
                                        </select>
                                    </div>
                                </div>
                ...
                <div className="xl:col-span-2 space-y-4">
                    ...
                    {constraints.length === 0 ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {constraints.map((c) => (
                                <div key={c._id} className="bg-white rounded-2xl shadow-sm border border-slate-200 p-5 hover:shadow-md transition-all group">
                                    <div className="flex justify-between items-start mb-3">
                                        <span className={`px-2.5 py-1 rounded-full text-xs font-bold ${c.type === 'preferred_off' ? 'bg-orange-50 text-orange-700' : 'bg-blue-50 text-blue-700'
                                            }`}>
                                            {c.type === 'preferred_off' ? 'Preferred Off' : 'Fixed Time'}
                                        </span>
                                        <button
                                    ...
                                    <div className="space-y-3">
                                        <div className="flex items-center gap-2 text-slate-700 border-b border-slate-50 pb-2">
                                            ...
                                            <span className="font-bold">{c.faculty_name || c.user_name}</span>
                                        </div>
                                        ...
                                        {c.type === 'fixed_time' && (
                                            <div className="grid grid-cols-1 gap-2">
                                                <div className="flex items-center gap-2 text-slate-600 text-sm">
                                                    ...
                                                    <span className="text-xs text-slate-400">({c.year}-{c.division})</span>
                                                </div>
                                            </div>
                                        )}
                                        ...
                                        <div className="flex items-center gap-2 text-slate-600 text-sm bg-slate-50 p-2 rounded-lg">
                                            ...
                                            <span className="font-medium">{c.day} at {c.time_slot}</span>
                                        </div>
